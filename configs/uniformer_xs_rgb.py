# UniFormer-XS (UniFormer_Light + FPN). Modality: RGB (3-band).
# Backbone values are taken from the training config that produced
# uniformer_xs_rgb.safetensors (type='UniFormer_Light', conv_stem, pruning).
...
model = dict(
    ...
    backbone=dict(
        type='UniFormer_Light', in_chans=3, conv_stem=True,
